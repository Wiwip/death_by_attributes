...
use crate::inspector::debug_overlay::pretty_type_name_str;
...
use ptree::{PrintConfig, Style, TreeBuilder, print_tree, write_tree};
use std::any::TypeId;
...
pub fn explore_actors_system(
    ...
    let _ = print_tree(&builder.build());
}
...
fn list_attributes(
    ...
    for (name, component_id, type_id, _) in actor_components.iter() {
...
fn list_effects(
    ...
    for effect_entity in actor_effects.iter() {
        let Ok((handle, name, modifiers)) = effect_query.get(effect_entity) else {
            continue;
        };
        let name = match name {
            None => "Effect",
...
fn list_modifiers_for_effect(
    ...
    // List attributes for printing
    for (name, component_id, type_id, _) in modifier_components.iter() {
        let Some(type_id) = type_id else {
            continue;
        };
        ...
        let registry = type_registry.read();
        let reflect_effect = registry.get(*type_id);
        let Some(reflect_effect) = reflect_effect else {
            continue;
        };
